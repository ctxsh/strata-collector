...
type DiscoveryLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Discovery, err error)
...
// List lists all Discoveries in the indexer.
func (s *discoveryLister) List(selector labels.Selector) (ret []*v1beta1.Discovery, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Discovery))
	})
	return ret, err
}
...
type DiscoveryNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Discovery, error)
...
// List lists all Discoveries in the indexer for a given namespace.
func (s discoveryNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Discovery, err error) {
...
// Get retrieves the Discovery from the indexer for a given namespace and name.
func (s discoveryNamespaceLister) Get(name string) (*v1beta1.Discovery, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("discovery"), name)
	}
	return obj.(*v1beta1.Discovery), nil
}
